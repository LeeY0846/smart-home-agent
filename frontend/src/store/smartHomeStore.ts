...
interface State {
  ...
  adjustDevice: (name: string, isPowerOn: boolean, value?: number) => void;
  addMessage: (msg: ChatMessage) => void;
  clearOngoingMessages: () => void;
...
export const useStore = create<State>((set) => ({
  ...
  adjustDevice: (name, isPowerOn, value) =>
    set((s) => ({
      devices: s.devices.map((d) => {
        if (d.name !== name) return d;
        const range = VALUE_RANGE[d.type];
        if (value !== undefined && range === undefined) return d;
        return {
          ...d,
